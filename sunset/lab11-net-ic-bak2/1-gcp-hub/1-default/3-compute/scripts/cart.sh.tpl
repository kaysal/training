...
while [ \$i -lt 8 ]; do
  ...
  ab -n ${n} -c ${c} http://en.wikipedia.org/ > /dev/null 2>&1
  let i=i+1
  sleep 1
done
EOF
...
touch /usr/local/bin/screenz
chmod a+x /usr/local/bin/screenz
cat <<EOF > /usr/local/bin/screenz
while true
do
  ab -n ${n} -c ${c} http://${TARGET}/
  ab -n ${n} -c ${c} http://en.wikipedia.org/
  sleep 1
done
EOF
