...
locals {
  onprem = { svc_account = data.terraform_remote_state.iam.outputs.svc_account.onprem }
  hub    = { svc_account = data.terraform_remote_state.iam.outputs.svc_account.hub }
  spoke1 = { svc_account = data.terraform_remote_state.iam.outputs.svc_account.spoke1 }
  spoke2 = { svc_account = data.terraform_remote_state.iam.outputs.svc_account.spoke2 }
}
...
resource "google_storage_bucket" "spoke1_bucket" {
  ...
  name          = "${var.global.prefix}${var.project_id_spoke1}-1"
...
resource "google_storage_bucket" "spoke2_bucket" {
  ...
  name          = "${var.global.prefix}${var.project_id_spoke2}-2"
  location      = "europe-west1"
  force_destroy = true
  storage_class = "REGIONAL"
}
