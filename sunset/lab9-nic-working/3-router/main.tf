...
resource "google_compute_router" "hub_router" {
  ...
  bgp {
    ...
    advertised_ip_ranges {
      range = local.hub.eu_subnet.ip_cidr_range
    }

    advertised_ip_ranges {
      range = local.hub.us_subnet.ip_cidr_range
    }

    advertised_ip_ranges {
      range = local.hub.asia_subnet.ip_cidr_range
    }

    advertised_ip_ranges {
      range = local.spoke2.eu_subnet.ip_cidr_range
    }

    advertised_ip_ranges {
      range = local.spoke2.asia_subnet.ip_cidr_range
    }
  }
}
